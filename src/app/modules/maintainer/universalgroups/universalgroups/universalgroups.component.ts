...
@Component({
  selector: 'app-universalgroups',
  templateUrl: './universalgroups.component.html',
  styleUrls: ['./universalgroups.component.css']
})
export class UniversalgroupsComponent implements OnInit, OnDestroy {
  ...
  redirectToEdit(element: ItemModel) {
    ...
    this.router.navigate(['/maintainer/add-upd-universalgroups']);
  }

  redirectToView(element: ItemModel) {
    this.elementService.elementSelected = element;
    this.router.navigate(['/maintainer/add-upd-universalgroups/ver']);
  }
    ...
    const resultModal = await this.modalService.open(
      {
        ...
        texto: `¿Esta seguro que desea eliminar el grupo "${elementSelected.description}"?`,
        icono: 'warning',
    ...
    if (resultModal) {
      ...
      this.subscriptions.push(this.elementService.delete(elementSelected).subscribe(async result => {
        ...
        const resultModal = await this.modalService.open(
          {
            titulo: 'Eliminado',
            texto: `El grupo "${elementSelected.description}" se ha eliminado correctamente.`,
            icono: 'success',
        ...
        if (err.error === MaintainerProperties.DEPENDENCY) {
          await this.modalService.open(
            {
              titulo: 'El grupo tiene dependencias',
              texto: 'Ya existen productos asociados a este grupo, debe eliminar los productos para poder eliminar el grupo.',
              icono: 'info',
              mostrarBotonCancelar: false,
