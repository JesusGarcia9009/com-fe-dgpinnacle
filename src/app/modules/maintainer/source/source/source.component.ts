...
export class SourceComponent implements OnInit , OnDestroy {
  ...
  async onDelete(elementSelected: ItemModel) {
    ...
    const resultModal = await this.modalService.open(
      {
        title: 'Eliminar el origen',
        text: `¿Esta seguro que desea eliminar el origen "${elementSelected.description}"?`,
        icon: 'warning',
    ...
    if (resultModal) {
      ...
      this.subscriptions.push(this.elementService.delete(elementSelected).subscribe(async result => {
        ...
        const resultModal = await this.modalService.open(
          {
            title: 'Eliminado',
            text: `El origen "${elementSelected.description}" se ha eliminado correctamente.`,
            icon: 'success',
        ...
        if (err.error === MaintainerProperties.DEPENDENCY) {
          await this.modalService.open(
            {
              title: 'El origen tiene dependencias',
              text: 'Ya existen productos asociados a este origen, debe eliminar los productos para poder eliminar el origen.',
              icon: 'info',
              showCancelButton: false,
              acceptText: 'Aceptar',
