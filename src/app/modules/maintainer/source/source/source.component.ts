...
export class SourceComponent implements OnInit , OnDestroy {
  ...
  constructor(
    ...
    private elementService: SourceService
  ) { }
  ...
  redirectToEdit(element: ItemModel) {
    ...
    this.router.navigate(['/maintainer/add-upd-source']);
  }

  redirectToView(element: ItemModel) {
    this.elementService.elementSelected = element;
    this.router.navigate(['/maintainer/add-upd-source/ver']);
  }
  ...
  async onDelete(elementSelected: ItemModel) {
    ...
    const resultModal = await this.modalService.open(
      {
        titulo: 'Eliminar el origen',
        texto: `¿Esta seguro que desea eliminar el origen "${elementSelected.description}"?`,
        icono: 'warning',
    ...
    if (resultModal) {
      ...
      this.subscriptions.push(this.elementService.delete(elementSelected).subscribe(async result => {
        ...
        const resultModal = await this.modalService.open(
          {
            titulo: 'Eliminado',
            texto: `El origen "${elementSelected.description}" se ha eliminado correctamente.`,
            icono: 'success',
        ...
        if (err.error === MaintainerProperties.DEPENDENCY) {
          await this.modalService.open(
            {
              titulo: 'El origen tiene dependencias',
              texto: 'Ya existen productos asociados a este origen, debe eliminar los productos para poder eliminar el origen.',
              icono: 'info',
              mostrarBotonCancelar: false,
