...
import { OrderNoteModel, OrderNoteProductModel, ReverseOrderNoteModel } from '../models/order-note.model';
...
@Component({
  selector: 'app-reverse-order-note',
  templateUrl: './reverse-order-note.component.html',
  styleUrls: ['./reverse-order-note.component.css']
})
export class ReverseOrderNoteComponent implements OnInit, OnDestroy {

  public formTitle: string = 'Registro de reversa de nota pedido';
  public reverseOrderNoteForm: FormGroup;
  ...
  initForm() {
    if (this.orderNote && this.orderNote.orderNoteStateName === 'FACTURADA') {
      this.reverseOrderNoteForm = this.fb.group({
        id: ['', []],
        idOrderNote: [0, []],
        numberOfCreditNote: ['', []],
        dateOfCreditNote: ['', []],
    ...
    this.subscriptions.push(
      zip(
        ...
        if (orderNote) {
          ...
          let formValue: ReverseOrderNoteModel = this.reverseOrderNoteForm.value;
          formValue.idOrderNote = orderNote.id;
          this.reverseOrderNoteForm.patchValue(formValue);
        }
      })
    );
  }
  ...
  get reverseOrderNoteFormControls() { return this.reverseOrderNoteForm.controls; }
  ...
  onRegisterSubmit() {
    this.loadingService.show();
    const formValue: ReverseOrderNoteModel = this.reverseOrderNoteForm.value;
    formValue.idOrderNote = this.orderNote.id;

    this.subscriptions.push(this.orderNoteService.reverse(formValue).subscribe(async response => {
      this.loadingService.hide();

      await this.modalService.open(
        {
          titulo: `Nota de pedido ${this.orderNote.id}`,
          texto: `La nota de pedido fue reversada correctamente.`,
          icono: 'success',
      ...
      this.reverseOrderNoteForm.reset();
